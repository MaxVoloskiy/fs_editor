...
void Image::edit(fileStruct file)
{
    ...
    for (int i = 0; i < 32; ++i)
    {
        imageData[catalogStart + i] = buff[i];
    }
    std::ofstream out("../new.img", std::ios::out | std::ios::binary);
    out.write(reinterpret_cast<char *>(imageData.data()), size);
    out.close();
}
